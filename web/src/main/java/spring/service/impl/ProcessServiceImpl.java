            ...
            synchronized (this.getClass()) {//线程
                ...
                if (StringUtils.isEmpty(taskId)) {//开启流程
                    ...
                    if (sysActivitiTasks == null || sysActivitiTasks.size() <= 0) {
                        ...
                        try {
                            params = JSONObject.parseObject(task.getFormKey()==null||"".equals(task.getFormKey())?JSON.toJSONString(new SysActivitiFormKey(),
                                    SerializerFeature.WriteMapNullValue):task.getFormKey(),
                        ...
                        if (params.get("isAutoDispose") == null || "1".equals(params.get("isAutoDispose"))) {
                            sysActivitiTaskServiceImpl.insert(new SysActivitiTask(IdWorker.getIdWorkerNext().toString(), null, null, null,
                                    null, IdWorker.getIdWorkerNext().toString()
                ...
                if (tlist != null && tlist.size() > 0) {
                    for (Task item : tlist) {
                        ...
                        try {
                            ...
                            List<SysActivitiTask> list =sysActivitiTaskServiceImpl.selectListByPage(sysActivitiTask1);
                            if (list!=null&&list.size() <= 0) {
                                String name1 = item.getAssignee() != null && !"".equals(item.getAssignee()) ? item.getAssignee() : null;
                                if(name1==null||"".equals(name1)){
                                    throw new CustomException("没找到任务处理人！");
                                }
